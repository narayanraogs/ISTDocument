...
class _ContentEditorScreenState extends State<ContentEditorScreen> {
  ...
  String _mapRouteToSubsection(String route) {
    ...
    switch (route) {
        // Introduction
        case 'intro_acronyms': return 'Introduction-Acronyms';
        case 'intro_subsystem_intro': return 'Introduction-SSIntroduction';
        case 'intro_subsystem_spec': return 'Introduction-SSSpecification';
        case 'intro_tc': return 'Introduction-Telecommand';
        case 'intro_tm': return 'Introduction-Telemetry';
        case 'intro_pages': return 'Introduction-Pages';
        
        // Checkout Details
        case 'checkout_interface': return 'Checkout-Interface';
        case 'checkout_specific_reqs': return 'Checkout-SpecificRequirements';
        case 'checkout_safety_reqs': return 'Checkout-SafetyRequirements';
        case 'checkout_test_philosophy': return 'Checkout-TestPhilosophy';
        case 'checkout_clarifications': return 'Checkout-SubsystemClarifications';
        
        // Test Details
        case 'test_matrix': return 'TestMatrix';
        case 'test_plan': return 'TestPlans';
        case 'test_procedure': return 'TestProcedures';
        
        // Annexure
        case 'annexure_eid': return 'Annexure-EID';
        case 'annexure_test_results': return 'Annexure-TestResultsFormat';

        // Information
        case 'info_signed_page': return 'Information-SignedPage';
  ...
  void _moveItem(int index, int delta) {
    ...
    setState(() {
      ...
      if (_editingIndex == index) _editingIndex = newIndex;
      else if (_editingIndex == newIndex) _editingIndex = index;
    });
  }
       ...
       Widget contentList = ListView(
         ...
         children: [
            Row(
              ...
              children: [
                Text(_mapRouteToSubsection(widget.subsectionKey), style: Theme.of(context).textTheme.headlineSmall),
                ElevatedButton.icon(onPressed: _saveData, icon: const Icon(Icons.save), label: const Text("Save All")),
              ],
            ),
            const Divider(),
            if (_items.isEmpty)
              const Center(child: Padding(padding: EdgeInsets.all(32), child: Text("No content yet. Add some!"))),

            for (int i = 0; i < _items.length; i++)
              ContentBlockWidget(
                key: ValueKey(_items[i]), // Using object identity as key? Better to have ID but index is okay if we are careful
                ...
                onMoveDown: i < _items.length - 1 ? () => _moveItem(i, 1) : null,
       ...
       if (isWide && _editingItem != null) {
         return Row(
           children: [
             ...
             Expanded(flex: 2, child: _buildEditorPane()), // Editor takes more space
           ],
         );
       }
  ...
  List<ContentType> _getAllowedTypes() {
    ...
    switch (widget.subsectionKey) {
      ...
      case 'intro_subsystem_spec':
        return [ContentType.text, ContentType.image, ContentType.table];
      case 'intro_tc':
      case 'intro_tm':
        return [ContentType.file, ContentType.code]; // File uploads
      case 'intro_pages':
      ...
      case 'checkout_clarifications':
        // Old client likely allowed Text/Image/Table here too?
        // Let's be permissive here or default to standard set
        return [ContentType.text, ContentType.image, ContentType.table];

      // Test Details
      case 'test_matrix': return [ContentType.table, ContentType.excel];
      case 'test_plan': return [ContentType.text, ContentType.image, ContentType.table];
      case 'test_procedure': return [ContentType.code, ContentType.file]; // Procedures
      ...
      itemBuilder: (context) {
        return [
          if (allowed.contains(ContentType.text))
            const PopupMenuItem(value: ContentType.text, child: Row(children: [Icon(Icons.text_fields), SizedBox(width: 8), Text('Text')])),
          if (allowed.contains(ContentType.image))
            const PopupMenuItem(value: ContentType.image, child: Row(children: [Icon(Icons.image), SizedBox(width: 8), Text('Image')])),
          if (allowed.contains(ContentType.table))
            const PopupMenuItem(value: ContentType.table, child: Row(children: [Icon(Icons.table_chart), SizedBox(width: 8), Text('Table')])),
          if (allowed.contains(ContentType.file))
            const PopupMenuItem(value: ContentType.file, child: Row(children: [Icon(Icons.picture_as_pdf), SizedBox(width: 8), Text('PDF')])),
          if (allowed.contains(ContentType.code))
            const PopupMenuItem(value: ContentType.code, child: Row(children: [Icon(Icons.code), SizedBox(width: 8), Text('Code/Procedure')])),
          if (allowed.contains(ContentType.excel))
            const PopupMenuItem(value: ContentType.excel, child: Row(children: [Icon(Icons.grid_on), SizedBox(width: 8), Text('Excel')])),
        ];
      },
    );
  }
  ...
  Widget _buildItemPreview(ContentItem item) {
    switch (item.type) {
      case ContentType.text:
        return MarkdownBody(data: item.value.isNotEmpty ? item.value : '(Empty Text)');
      case ContentType.image:
        if (item.value.isEmpty) return const Text('No Image Selected');
        try {
           return Column(
             children: [
               ConstrainedBox(constraints: const BoxConstraints(maxHeight: 200), child: Image.memory(base64Decode(item.value))),
               if (item.caption.isNotEmpty) Text(item.caption, style: Theme.of(context).textTheme.bodySmall),
             ],
           );
        } catch (e) { return const Text('Invalid Image Data'); }
      case ContentType.table:
         return Column(children: [
            const Icon(Icons.table_chart, size: 48, color: Colors.grey),
            Text('Table: ${item.caption}'),
            Text(item.value, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(fontFamily: 'monospace')),
         ]);
      default:
        // Generic preview for files/pdf
        return Column(children: [
           Icon(item.type == ContentType.file ? Icons.picture_as_pdf : Icons.data_object, size: 48, color: Colors.red.shade700),
           Text(item.fileName.isNotEmpty ? item.fileName : (item.type == ContentType.file ? 'PDF Document' : item.typeString)),
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Text("Editing ${_editingItem!.type == ContentType.file ? 'PDF' : _editingItem!.typeString}", style: Theme.of(context).textTheme.headlineSmall),
              const Spacer(),
  ...
  void _showEditDialog(BuildContext context) {
    showDialog(context: context, builder: (context) {
       return Dialog.fullscreen(
         child: Scaffold(
           appBar: AppBar(
             title: Text("Edit ${_editingItem!.type == ContentType.file ? 'PDF' : _editingItem!.typeString}"),
             actions: [
           ...
           body: Padding(
             ...
             child: SingleChildScrollView(child: _buildSpecificEditor(_editingItem!)),
           ),
         ),
       );
    }).then((_) {
  ...
  Widget _buildSpecificEditor(ContentItem item) {
    ...
    if (item.type == ContentType.text) {
      children.add(TextEditor(initialValue: item.value, onChanged: (v) {
        item.value = v; 
      }));
      // Text doesn't need caption or landscape
       ...
       children.add(TextField(
         ...
         decoration: const InputDecoration(labelText: 'Raw Value / Content', border: OutlineInputBorder()),
       ));
       _addCommonFields(children, item);
    }
  ...
  Widget _buildExcelEditor(ContentItem item) {
    return Column(
      ...
      children: [
        const Text("Excel Spreadsheet", style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        Container(
          ...
          child: Column(
             children: [
                ...
                const SizedBox(height: 8),
                Text(
                  item.fileName.isNotEmpty ? item.fileName : "No Excel File Selected",
                  style: const TextStyle(fontWeight: FontWeight.bold),
                ),
                Text(
                  item.value.isNotEmpty ? "${(item.value.length / 1024).toStringAsFixed(1)} KB" : "",
  ...
  Widget _buildTableEditor(ContentItem item) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("Table Content (CSV)", style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Container(
            ...
            if (result != null) {
              ...
              if (bytes != null) {
                ...
                try {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Error reading CSV file")));
                }
              }
            }
          },
  ...
  Widget _buildFileEditor(ContentItem item) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text("Procedure/Code File", style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Container(
          padding: const EdgeInsets.all(16),
          width: double.infinity,
